...
def load_telephone(*, as_frame=False):
    ...
    with open(DESCR_FOLDER / "telephone.rst", "r") as f:
        fdescr = f.read()
...
def load_stars(*, as_frame=False):
    ...
    df = pd.read_csv(DATA_FOLDER / data_file_name)
    with open(DESCR_FOLDER / "stars.rst", "r") as f:
        fdescr = f.read()
...
def load_animals(*, as_frame=False):
    ...
    df = pd.read_csv(DATA_FOLDER / data_file_name)
    with open(DESCR_FOLDER / "animals.rst", "r") as f:
        fdescr = f.read()
...
def load_topgear(*, as_frame=False):
    ...
    >>> mm = FastLTSRegressor().fit(data.data[["Cylinders", "Torque", "TopSpeed"]], data.data["Price"])
    """
    data_file_name = "topgear.csv"
    df = pd.read_csv(DATA_FOLDER / data_file_name)
    with open(DESCR_FOLDER / "topgear.rst", "r") as f:
        fdescr = f.read()
...
def load_glass(*, as_frame=False):
    ...
    df = pd.read_csv(DATA_FOLDER / data_file_name)
    with open(DESCR_FOLDER / "glass.rst", "r") as f:
        fdescr = f.read()
