...
class RobustPowerTransformer(OneToOneFeatureMixin, TransformerMixin, BaseEstimator):
    ...
    features closer to normality. Uses the Box-Cox or the Yeo-Johnson transformation.
        ...
        Args:
            method (Literal str, optional): method used for the power transformation. Can be
                    "boxcox" for Box-Cox, "yeojohnson" for Yeo-Johnson, or "auto" for best objective
                    solution. Box-Cox can only be used for strictly positive features.
                    Defaults to "auto".
            standardize (boolean, optional): whether to standardize the features before and after
                    the power transformation.
                    Defaults to True.
            quantile (float, optional): quantile used to calculate the weights.
                    Defaults to 0.99.
    ...
    def transf_boxcox_rectified(
        ...
        if my_lambda == 1.0:
            ...
            changepoint = self.get_rectified_boxcox_changepoint(x, my_lambda)
    ...
    def get_rectified_boxcox_changepoint(
        ...
        if my_lambda < 1:
            changepoint = self.transf_boxcox(Q3, my_lambda)[0] * factor
        elif my_lambda > 1:
            changepoint = self.transf_boxcox(Q1, my_lambda)[0] * factor
        else:
        ...
        changepoint = self.inv_transf_boxcox(changepoint, my_lambda)
    ...
    def transf_yeojohnson_rectified(
        ...
        if my_lambda == 1.0:
            ...
            changepoint = self.get_rectified_yeojohnson_changepoint(x, my_lambda)
            positive_mask = x >= 0.0
            negative_mask = x < 0.0
            ...
            if my_lambda > 1.0:
                ...
                if my_lambda == 2:
                    ...
                    changepoint_transf = self.transf_yeojohnson(changepoint, my_lambda)[0]
    ...
    def get_rectified_yeojohnson_changepoint(
        ...
        if my_lambda < 1:
            changepoint = self.transf_yeojohnson(Q3, my_lambda)[0] * factor
        elif my_lambda > 1:
            changepoint = self.transf_yeojohnson(Q1, my_lambda)[0] * factor
        else:
            raise ValueError("There is no changepoint for lambda equals 1.")
        ...
        changepoint = self.inv_transf_yeojohnson(changepoint, my_lambda)
        changepoint = min(max(changepoint, x[0]), x[n - 1])
    ...
    def robnormality(
        ...
        if transf == "boxcox_rect":
            x = self.transf_boxcox_rectified(x, my_lambda)[0]
        elif transf == "yeojohnson_rect":
            x = self.transf_yeojohnson_rectified(x, my_lambda)[0]
        elif transf == "boxcox":
            x = self.transf_boxcox(x, my_lambda)[0]
        elif transf == "yeojohnson":
            x = self.transf_yeojohnson(x, my_lambda)[0]
        else:
    ...
    def calculate_lambda_0(
        ...
        lambda_0 = minimize_scalar(
            lambda lambdatemp: self.robnormality(x, transf, lambdatemp),
            bounds=lambdarange,
            method="bounded",
        )
    ...
    def reweighted_max_likelihood_robust(
        ...
        if transf == "boxcox":
            zt = self.transf_boxcox_rectified(x, lambda_raw, standardize_too=True)[1]
        elif transf == "yeojohnson":
            zt = self.transf_yeojohnson_rectified(x, lambda_raw, standardize_too=True)[1]

        for _ in range(nsteps):
            w = np.abs(zt) <= np.sqrt(chi2.ppf(quantile, 1))
            lambda_rew = self.estimate_max_likelihood(x[w], transf, lambdarange)
            if transf == "boxcox":
                xt = self.transf_boxcox(x, lambda_rew, standardize_too=False)[0]
            elif transf == "yeojohnson":
    ...
    def estimate_max_likelihood(
        ...
        if transf == "boxcox":
            ...
            def obj_func(lambdatemp):
                x_bc = self.transf_boxcox(x, lambdatemp)[0]
                mu = np.mean(x_bc)
            ...
            def obj_func(lambdatemp):
                x_yj = self.transf_yeojohnson(x, lambdatemp)[0]
                mu = np.mean(x_yj)
